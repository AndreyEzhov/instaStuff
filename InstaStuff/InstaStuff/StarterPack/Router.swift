...
extension Router {
    ...
    func updateHierarchy() {
        ...
        if let usersTempalte = (viewControllers.filter { $0 is TemplatePickerController }.first as? TemplatePickerController), usersTempalte.presenter.usersTemplate == true {
            viewControllers = [usersTempalte, lastViewController]
        } else {
